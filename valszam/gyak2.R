...
# értelemben (egyelõre) nem állíthatjuk róla, hogy konvergens. Csupán annyi 
...
freqs <- table(dobasok)   # fejek és írások gyakorisága n dobás esetén
                          #  table(): excel pivot tábla megfelelõje
freqs

fejgyak <- cumsum(dobasok == 'F') / 1:ndobas  # fejek és írások gyakorisága minden n-re
              # ez a hányados maga a relatív gyakoriság
...
##################################################################################
######################## Diszkrét valószínûségi változók #########################
##################################################################################
...
# Négy egyforma érmét feldobva a megfigyelhetõ kimenetelek: 
# 4 fej,   3 fej és 1 írás,   2 fej és 2 írás,   1 fej és 3 írás,   4 írás
# Ezek valószínûsege a 16-elemû eseménytéren kombinatorikus valószínûseggel 
...
#k kimenetelk, aminek valószínüségét akarjuk kiszámolni (0,1,2,3,4)
...
# bekövetkezésük valószínûsége p ill. 1-p. 
# Ezt a kísérletet (érmedobás) egymástól függetlenül n-szer végrehajtjuk. Ha csak F gyakoriságát 
# figyeljük (jelölje ezt X), akkor (n+1)-féle eredményt kaphatunk, amelynek valószínûsége
# P(X = k) = (n alatt k) p^k (1-p)^(n-k), ahol k=0,1,...,n
# => kaptunk egy (n+1)-elemû eseményteret a fenti eloszlással: binomiális eloszlás
...
# kísérlet: érmedobás, fejek ill. írások valószínûsége
# P(X = 1) = p, P(X = 0) = 1 - p 


#pl.: van egy adott méretû pocsolyánk, adott idõ alatt hány csepp esik bele
...
#készítsünk az elõzõ ábrához hasonlót, ahol a binomiálist
...
# Tegyük fel, hogy az új internet-elõfizetõk véletlenszerûen választott 20%-a 
# speciális kedvezményt kap. Mi a valószínûsége, hogy 10 ismerõsünk közül, 
# akik most fizettek elõ, legalább négyen részesülnek a kedvezményben?
...
# Mi a valószínûsége annak, hogy egy 1 m^2-es mintában
