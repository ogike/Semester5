...
# 1. Egy szabályos dobókockával négyszer dobtunk és a következõket kaptuk: 1, 3, 6, 1.
...
sd(x)                       # korrigált tapasztalati szórás ( = sqrt(var(x)) ) 
                              # "torzítatlan becslése a valóságos szórásnak"
...
# (b) Tekintsük a 101, 103, 106, 101 adatokat, melyeket az elozõekbõl
...
#     Hibásnak tûnik két adat, azokat jelen esetben értelemszerûen 
...
summary(radatok)      # statisztikák: min, max, átlag, kvartilisek
                        # kvartilis: medián általánosítása
data.entry(radatok)

# Javított magassági adatok:
adatok <- radatok
adatok[3] <- 150	# ellenõrízve: 1500 helyett 150
adatok[11] <- 168	# ellenõrízve: 1-68 helyett 168
adatok
...
# kvartilisek kiszámítása, 6-os típus: (elméleti leírásban szereplõ képlet alapján) 
...
h <- hist(airquality$Temp, #breaks=seq(50,100,2), #breaks=seq(50,100,10),
          ...
          xlab="hõmérséklet (F)", 
          ylab="gyakoriság", 
          main="Hõmérséklet hisztogram (airquality)")
h$breaks
h$counts
...
########## MEGOLDÁS ##########
# A table függvény elõször az adat vektorból készít kontigencia táblát. 
# Azaz az elõforduló értékek mindegyikére megadja az elõfordulások számát. 
# Ezután kiírja a leggyakrabban elõforduló értékeket (string-ként).
##############################
...
########## MEGOLDÁS ##########
# Az elsõ két sor egy data.frame-et készít. Az adatok elsõ fele A, 
...
# a korrigálatlan változatot számolja, így ezek általában különbõzõek 
...
# Minta: X_1,...,X_n valószínûségi változó sorozat.  
...
# az F(x) elméleti eloszlásfüggvény közötti eltérés maximuma 1 valószínûséggel 
